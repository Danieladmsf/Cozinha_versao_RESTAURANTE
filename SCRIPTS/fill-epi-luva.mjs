// Script para preencher o POP EPI0001 existente
// Execute com: node SCRIPTS/fill-epi-luva.mjs
...
async function fillExistingPOP() {
    console.log('🔍 Buscando TODOS os documentos na coleção "epis"...\n');
    ...
    try {
        ...
        for (const docSnap of snapshot.docs) {
            ...
            // Atualizar se nome contém "Luva" ou "Malha" ou codigo EPI0001
            if (data.nome?.toLowerCase().includes('luva') ||
                data.nome?.toLowerCase().includes('malha') ||
                data.codigo === 'EPI0001' ||
                !data.especificacoes) { // ou se não tem dados

                console.log(`   ➡️ Atualizando este documento...`);
