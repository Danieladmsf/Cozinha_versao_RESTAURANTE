...
async function runQuery() {
    return new Promise((resolve, reject) => {
        ...
        // Query to find tables related to sales/pdv and inspect 'venda' table
        const sql = `
            SELECT table_name 
            FROM information_schema.tables 
            WHERE table_schema = 'public' 
            AND (table_name ILIKE '%venda%' OR table_name ILIKE '%pdv%' OR table_name ILIKE '%cupom%')
            ORDER BY table_name;
        `;
                ...
                switch (type) {
                    ...
                    case 'Z':
                        if (step === 'startup') {
                            step = 'ready';
                            if (!querySent) {
                                console.log('Searching for sales tables...');
