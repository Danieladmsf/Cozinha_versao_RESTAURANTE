...
(async () => {
    try {
        console.log("== HUNTING FOR 6025.85 ==");
        
        // Sum by day for Rotisseria
        const q1 = `
            SELECT data::text, sum(valortotal) as venda_dia 
            FROM venda v JOIN produto p ON p.id = v.id_produto
            WHERE v.data >= '2026-04-01' AND v.data <= '2026-04-05' AND p.mercadologico1 = 17
        ...
        for(const r of r1.rows) {
            acc += parseFloat(r.venda_dia);
            console.log(r.data, acc);
        }
