...
(async () => {
    // Pegando apenas o setor ROTISSERIA (mercadologico1 = 17) para os últimos 12 meses
    const sql = `
        SELECT 
            TO_CHAR(v.data, 'YYYY-MM') as mes,
            SUM(v.valortotal) as venda_mensal
        FROM venda v
        JOIN produto p ON p.id = v.id_produto
        WHERE v.data >= '2025-03-01' AND v.data <= '2026-03-31'
        AND p.mercadologico1 = 17 
        GROUP BY 1
        ORDER BY 1 DESC;
    `;
    ...
    try {
        ...
        console.log("=== VENDAS ROTISSERIA (ÚLTIMOS 12 MESES) ===");
        console.log("");
        
        for (const row of results) {
            console.log(`*${row.mes.padEnd(10, '.')}* ${formatCurrency(row.venda_mensal)}`);
        }
    } catch(e) {
