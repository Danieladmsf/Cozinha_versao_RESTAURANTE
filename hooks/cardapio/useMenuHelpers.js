...
export const useMenuHelpers = () => {
  ...
  const filterRecipesBySearch = useCallback((recipes, categoryName, searchTerm, categories = []) => {
    ...
    const filteredRecipes = availableRecipes.filter(recipe => {
      ...
      const normalizedRecipeCode = recipe.code ? normalizeText(String(recipe.code)) : '';

      // Check simple match (name OR code)
      if (!normalizedSearchTerm.includes(' ')) {
        return normalizedRecipeName.includes(normalizedSearchTerm) ||
          normalizedRecipeCode.includes(normalizedSearchTerm);
      }

      // Check multiple words (any word match in name OR code)
      const searchWords = normalizedSearchTerm.split(/\s+/).filter(word => word.length > 0);

      return searchWords.some(word =>
        normalizedRecipeName.includes(word) || normalizedRecipeCode.includes(word)
      );
    });
