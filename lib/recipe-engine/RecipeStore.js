...
const useRecipeStore = create((set, get) => ({
  ...
  saveRecipe: async (recipeData, preparationsData) => {
    ...
    try {
      const recipeToSave = {
        ...
        operational_cost: parseFloat(get().metrics.operational_cost) || 0, // COO
  ...
  validate: () => {
    ...
    if (!recipe.name || recipe.name.trim() === '') {
      errors.name = 'O nome da receita é obrigatório';
      isValid = false;
    }

    if (!recipe.category || recipe.category.trim() === '') {
      errors.category = 'A categoria da receita é obrigatória';
      isValid = false;
    }

    if (!state.preparations || state.preparations.length === 0) {
      errors.preparations = 'A receita deve ter pelo menos uma etapa de preparo';
      isValid = false;
    }
