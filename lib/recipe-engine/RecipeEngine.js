...
export const WEIGHT_FIELDS = {
  ...
  INITIAL_PRIORITY: ['weight_frozen', 'weight_raw', 'weight_thawed', 'weight_clean', 'weight_pre_cooking', 'quantity'],
...
export class RecipeEngine {
  ...
  static getInitialWeight(ingredient, processes = []) {
    ...
    // 1. Se informaram processos específicos, tentar os campos de "entrada" desses processos primeiro
    if (processes && Array.isArray(processes) && processes.length > 0) {
      // Prioridade: Thawing -> Cleaning -> Cooking -> Portioning
      if (processes.includes('defrosting')) {
        const val = this.parseValue(ingredient.weight_frozen);
        if (val > 0) return val;
      }
      
      if (processes.includes('cleaning')) {
        const val = this.parseValue(ingredient.weight_raw) || this.parseValue(ingredient.weight_thawed);
        if (val > 0) return val;
      }
      
      if (processes.includes('cooking')) {
        const val = this.parseValue(ingredient.weight_pre_cooking) || this.parseValue(ingredient.weight_clean);
        if (val > 0) return val;
      }
    }

    // 2. Fallback: Procurar em todos os campos possíveis na ordem lógica de prioridade
    for (const field of WEIGHT_FIELDS.INITIAL_PRIORITY) {
      const parsed = this.parseValue(ingredient[field]);
  ...
  static getUnitPrice(ingredient) {
    ...
    for (const field of PRICE_FIELDS.PRIORITY) {
      if (this._hasExplicitValue(ingredient[field])) {
        return this.parseValue(ingredient[field]);
      }
    }
  ...
  static calculatePreparationMetrics(preparation, allRecipes = []) {
    ...
    if (!custouDaMatriz && normalizedPrep.ingredients && normalizedPrep.ingredients.length > 0) {
      normalizedPrep.ingredients.forEach(ingredient => {
        ...
        // NOVO: Ignorar insumos de Imersão/Marinada do rendimento físico da receita (mas manter o custo)
        const isYieldExcluded = ingredient.name && (
          ingredient.name.toLowerCase().includes('(fritura)') ||
          ingredient.name.toLowerCase().includes('(imers') ||
          ingredient.name.toLowerCase().includes('(marinada)')
        );
        ...
        // O custo sempre é somado à receita!
        totalCost += cost;
      });
    }
    ...
    if (normalizedPrep.sub_components && normalizedPrep.sub_components.length > 0) {
      normalizedPrep.sub_components.forEach(sc => {
        ...
        const isPkg = sc.isPackaging || sc.is_packaging || (sc.name && sc.name.toLowerCase().includes('embalagem'));
        if (!isPkg) {
  ...
  static calculateRecipeMetrics(recipeData, preparationsData = [], allRecipes = []) {
    ...
    // Verificar se a receita raiz tem ingredientes base ou sub-receitas.
    // Se tiver, as etapas de montagem (assembly/portioning) NÃO devem somar ao totalRawWeight
    // para evitar contagem dupla (Ingredientes + Produto Final).
    const hasBaseIngredients = normalizedPreparations.some(p => p.ingredients?.length > 0 || p.recipes?.length > 0);
    ...
    normalizedPreparations.forEach((prep, index) => {
      ...
      // Somar apenas preparações válidas
      if (hasValidItems) {
        ...
        if (isPackaging) {
          ...
          // Se for etapa de montagem e a receita já tem ingredientes raízes, NÃO soma o peso bruto
          // da montagem, pois os ingredientes já contam como o peso bruto total.
          if (!(hasBaseIngredients && isAssemblyOrPortioning)) {
  ...
  static calculatePortionMetrics(preparations, costPerKgYield) {
    ...
    // Encontrar a última preparação de assembly/portioning, pois ela é o output real
    const assemblyPrep = preparations.slice().reverse().find(prep =>
  ...
  // ========================================
  // COMPATIBILIDADE - APELIDOS PARA CÓDIGO ANTIGO
  // ========================================
  
  static parseNumericValue(val) { return this.parseValue(val); }
  
  static calculateThawingLoss(item) { return this.calculateAndClassifyThawingLoss(item).value; }
  static calculateCleaningLoss(item) { return this.calculateAndClassifyCleaningLoss(item).value; }
  static calculateCookingLoss(item) { return this.calculateAndClassifyCookingLoss(item).value; }
  static calculatePortioningLoss(item) { return this.calculateAndClassifyPortioningLoss(item).value; }
  
  static calculateItemYieldPercent(item) { return this.calculateIngredientYield(item); }
  static calculateItemNetPricePerKg(item) { return this.calculateItemLiquidPrice(item); }
}
...
export const parseNumericValue = RecipeEngine.parseNumericValue;
export const calculateThawingLoss = RecipeEngine.calculateThawingLoss;
export const calculateCleaningLoss = RecipeEngine.calculateCleaningLoss;
export const calculateCookingLoss = RecipeEngine.calculateCookingLoss;
export const calculatePortioningLoss = RecipeEngine.calculatePortioningLoss;
export const calculateItemYieldPercent = RecipeEngine.calculateItemYieldPercent;
export const calculateItemNetPricePerKg = RecipeEngine.calculateItemNetPricePerKg;
