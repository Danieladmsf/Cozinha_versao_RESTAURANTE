  ...
  static getInitialWeight(ingredient, processes = []) {
    ...
    if (processes.length > 0) {
      ...
      if (processes.includes('cleaning')) {
        if (this._hasExplicitValue(ingredient.weight_raw)) return this.parseValue(ingredient.weight_raw);
        if (this._hasExplicitValue(ingredient.weight_thawed)) return this.parseValue(ingredient.weight_thawed);
      }

      // Para defrosting, usar weight_frozen
      if (processes.includes('defrosting')) {
        if (this._hasExplicitValue(ingredient.weight_frozen)) return this.parseValue(ingredient.weight_frozen);
      }
    }
