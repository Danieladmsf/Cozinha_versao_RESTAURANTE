 ...
 */
export function calculateRecipeMetrics(preparationsData, recipeData = {}, allRecipes = []) {
  // Usar o sistema unificado - CORREÇÃO DE ORDEM DOS PARÂMETROS
  const result = RecipeCalculator.calculateRecipeMetrics(recipeData, preparationsData, allRecipes);
 ...
 */
export function updateRecipeMetrics(preparationsData, currentMetrics = {}, recipeData = {}, allRecipes = []) {
  ...
  const newMetrics = calculateRecipeMetrics(updatedPreparations, recipeData, allRecipes);
...
export { parseValue as parseNumericValue } from '@/lib/recipe-engine/RecipeEngine';
export { formatters } from '@/lib/recipe-engine/RecipeEngine';
