import { RecipeEngine } from '../recipe-engine/RecipeEngine.js';
...
export class DemandCalculator {
    ...
    static explodeOrders(orders, allRecipes, categoryMap) {
        ...
        orders.forEach(order => {
            ...
            order.items.forEach(orderItem => {
                ...
                // FIX: If the found entity is a Product (or just lacks preparations) but has a linked base recipe, resolve it
                let baseRecipe = recipe;
                if (!recipe.preparations && recipe.recipe_id) {
                    baseRecipe = allRecipes.find(r => r.id === recipe.recipe_id) || recipe;
                } else if (!recipe.preparations && !recipe.recipe_id) {
                    baseRecipe = allRecipes.find(r => r.name === recipe.name && r.id !== recipe.id && r.preparations) || recipe;
                }

                if (!baseRecipe.preparations) return;

                const orderedQty = this.parseNumber(orderItem.quantity);
                if (orderedQty <= 0) return;

                const recipeYieldWeight = this.getRecipeYieldWeight(baseRecipe, allRecipes);
                if (recipeYieldWeight <= 0) return;

                let unitsQuantity = 1;
                const lastPrep = baseRecipe.preparations[baseRecipe.preparations.length - 1];
                ...
                let orderUnitType = (orderItem.unit_type || recipe.unit_type || recipe.container_type || '').toLowerCase();

                if (['porção', 'porcao', 'un', 'unidades'].includes(orderUnitType)) {
                    orderUnitType = 'unidade';
                }

                let isSoldByWeight = false;

                // --- HEURÍSTICA DE PROTEÇÃO (VR PDV) ---
                // O PDV ocasionalmente exporta Marmitas ou Refeições com a tag de 'kg' mas a quantity é nitidamente em Unidades (ex: 8, 10).
                const nameLower = (recipe.name || '').toLowerCase();
                const isExplicitlyUnit = nameLower.includes('marmita') ||
                    nameLower.includes('refeicao') ||
                    nameLower.includes('refeição') ||
                    nameLower.match(/ un$/) ||
                    nameLower.includes('kit');

                if (isExplicitlyUnit) {
                    isSoldByWeight = false;
                } else if (orderUnitType === 'kg' || orderUnitType.includes('cuba')) {
                    isSoldByWeight = true;
                } else if (orderUnitType === 'unidade' || orderUnitType === 'emb' || orderUnitType === 'embalagem') {
                    isSoldByWeight = false;
                } else {
                    isSoldByWeight = (assemblyUnitType === 'kg');
                }
                ...
                let categoryName = baseRecipe.category || recipe.category || 'Outros';
                ...
                } else if (baseRecipe.category_id && categoryMap.has(baseRecipe.category_id)) {
                    const catData = categoryMap.get(baseRecipe.category_id);
                    categoryName = (typeof catData === 'object' && catData?.name) ? catData.name : (catData || categoryName);
                ...
                const contextStr = `${baseRecipe.name} (${order.customer_name || 'Geral'})`;
                ...
                this.recursiveExplode({
                    ...
                    onLeafFound: (ing, scaledQty, context, originId) => {
                        ...
                        leafIngredients.push({
                            ...
                            topLevelRecipeId: baseRecipe.id,
                            topLevelRecipeName: baseRecipe.name,
                            topLevelOrderedQty: orderedQty,
                            topLevelIsSoldByWeight: isSoldByWeight,
                            topLevelAssemblyUnit: assemblyUnitType || orderUnitType || 'un',
                            topLevelPortionWeight: baseRecipe.portion_weight_calculated || recipe.portion_weight_calculated || 0
                        });
                    }
                });
            });
    ...
    static processIngredientOrExplode({ ing, parentScale, allRecipes, contextStr, visitedIds, originId, onLeafFound }) {
        ...
        const qtyYield = isPkg ? qtyRaw : RecipeEngine.getFinalWeight(ing);
        const scaledYieldQty = qtyYield > 0 ? qtyYield * parentScale : scaledQty;

        // 🔍 DEBUG: Verificar folha final
        if ((ingName || '').toLowerCase().includes('berinjela')) {
            console.log('🔍 [BERINJELA LEAF DEBUG]', {
                ingName,
                qtyRaw,
                parentScale,
                scaledQty,
        ...
        // Resolve task_type with fallback (if this was an implicit sub-recipe, the task_type might be lost)
        let resolvedTaskType = ing.task_type;
        if (resolvedTaskType === undefined && type === 'food' && originId) {
            const baseRecipe = allRecipes.find(r => r.id === originId);
            if (baseRecipe && baseRecipe.preparations) {
                for (const bp of baseRecipe.preparations) {
                    ...
                    if (foundIng && foundIng.task_type !== undefined) {
                        resolvedTaskType = foundIng.task_type;
                        break;
                    }
                }
            }
        ...
        // Passa adiante com a flag de tipo e task_type adicionada dinamicamente ao objeto 'ing' para reports
        const enrichedIng = { ...ing, item_type: type, canonical_unit: unit, task_type: resolvedTaskType, _scaledYieldQty: scaledYieldQty };
