...
export class OrderSuggestionManager {
  ...
  static calculateShelfLifeAverage(eventsByDate, targetDayOfWeek, shelfLife, salesWindow) {
    ...
    // Trava de "Gordura de Quebra": Como a janela corta e descarta o resto das vendas (Sobra na panela)
    // Se a venda tem Janela de tempo, sugerir sempre +3 ou +4 itens para não faltar para os últimos clientes
    if (salesWindow !== 'all_day' && avg > 0) {
      avg += 3; // Segurança de Quebra de Fim do Expediente
    }
