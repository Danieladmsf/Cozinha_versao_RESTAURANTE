...
export class OrderSuggestionManager {
   ...
   */
  static async loadRecipeAdjustments(recipeIds, preloadedRecipes = null) {
    const adjustments = {};

    try {
      let recipes = [];
      if (preloadedRecipes && preloadedRecipes.length > 0) {
        // Find recipes from the preloaded list to avoid N+1 queries
        recipes = recipeIds.map(id => preloadedRecipes.find(r => r.id === id) || null);
      } else {
        // Fallback: Carregar receitas em paralelo
      ...
      if (anchorDate.getDay() === jsTargetDay) {
        ...
        for (let i = 0; i < shelfLife; i++) {
          ...
          if (dayEvents) {
            ...
            if (i === shelfLife - 1 && salesWindow && salesWindow !== 'all_day' && salesWindow !== 'Dia Todo') {
              let startHour = 0;
              let endHour = 24;
  ...
  static async generateOrderSuggestions(customerId, currentOrderItems, mealsExpected, options = {}) {
    ...
    try {
        ...
        try {
          ...
          recipesToScan.forEach(r => {
            ...
            if (code) {
              ...
              if (!isNaN(codeInt)) {
                // Adicionar variações do código (c/ e s/ zeros) para garantir match no SalesHistory
                const variations = [
                  codeStr,
                  String(codeInt),
                  String(codeInt).padStart(4, '0'),
                  String(codeInt).padStart(6, '0')
                ];

                variations.forEach(v => {
                  productCodesSet.add(v);
                  if (!recipeMap[v]) recipeMap[v] = [];
                  const rId = r.recipe_id || r.id;
                  if (!recipeMap[v].includes(rId)) recipeMap[v].push(rId);
                });
              }
            }
          });
          ...
          const codeStrings = Array.from(productCodesSet);
          let rawSales = [];
          ...
          if (rawSales.length > 0) {
            ...
            rawSales.forEach(doc => {
              ...
              } else if (doc.events && typeof doc.events === 'object') {
                // Suporte para quando events for um Map/Object: { "13": 5, "14": 12 }
                Object.entries(doc.events).forEach(([hour, qty]) => {
                  if (qty !== undefined) {
                    // Tratar tanto { "8": { qty: 5 } } quanto { "8": 5 }
                    hourlyQty[String(hour)] = typeof qty === 'object' ? (qty.qty || 0) : qty;
                  }
                });
              }

              if (Object.keys(hourlyQty).length === 0 && doc.total_quantity !== undefined) {
            ...
            currentOrderItems.forEach(item => {
              const codeIntStr = item.code || item.product_code || item.external_code || item.vr_product_code;
              const codeInt = parseInt(codeIntStr, 10);
              if (isNaN(codeInt)) return;

              // Tentar encontrar os dados de venda mapeados usando as variações do código
              const codeStr = String(codeIntStr).trim();
              const possibleCodes = [
                codeInt,
                codeStr,
                String(codeInt),
              ...
              for (const pc of possibleCodes) {
                if (mappedSales[pc]) {
                  codeInfo = mappedSales[pc];
                  matchedCode = pc;
                  break;
                }
              }

              const isDebugItem = codeInt === 7875 || codeInt === 1;
              if (isDebugItem) {
                console.log(`[DEBUG-SUG] Avaliando Item: ${item.name} | Code: ${codeIntStr} -> Encontrou no SalesHistory? ${!!codeInfo} (Matched API ID: ${matchedCode})`);
              }
              ...
              if (codeInfo && Object.keys(codeInfo.events).length > 0) {
                ...
                if (isDebugItem) {
                  console.log(`[DEBUG-SUG] ${item.name} | targetDayOfWeek: ${dayOfWeek} | shelfLife: ${shelfLife} | salesWindow: ${salesWindow}`);
                }
                ...
                if (isDebugItem) {
                  console.log(`[DEBUG-SUG] Resultado do cálculo para ${item.name}:`, smartStats);
                }
      ...
      const adjustments = await this.loadRecipeAdjustments(recipeIds, options.fullRecipes || []);
