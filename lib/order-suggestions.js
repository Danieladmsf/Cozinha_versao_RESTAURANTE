...
export class OrderSuggestionManager {
  ...
  static async generateOrderSuggestions(customerId, currentOrderItems, mealsExpected, options = {}) {
    ...
    try {
      ...
      // === ESTRATÉGIA 1: USAR CACHE FIREBASE (vr_sales_sync) ===
      if (useVrSales) {
        console.log(`🔍 [Suggestion] USANDO APENAS CACHE FIREBASE (vr_sales_sync)...`);
        try {
          // 1. Coletar códigos de produtos
          const productCodes = [];
          const recipeMap = {}; // map code -> [recipeId]

          // Usar fullRecipes se disponível, senão tentar extrair de currentOrderItems
          ...
          recipesToScan.forEach(r => {
            ...
            if (code) {
              ...
              if (!isNaN(codeInt)) {
                ...
                if (!recipeMap[codeInt]) {
                  recipeMap[codeInt] = [];
                }

                const rId = r.recipe_id || r.id;
                if (!recipeMap[codeInt].includes(rId)) {
                  recipeMap[codeInt].push(rId);
                }
              }
            }
          });

          // 2. Buscar snapshot direto do Firebase
          const snapshot = await VrSalesSync.getById('current_snapshot');

          if (snapshot) {
            let salesMap = null;

            if (snapshot.strategy === 'chunked' || snapshot.totalChunks > 0) {
              // Logic to fetch chunks for suggestion system
              console.log(`📦 [Suggestion] Detectado snapshot chunked (${snapshot.totalChunks} chunks).`);

              // Need to fetch chunks manually since VrSalesSync is generic
              // We can loop getById('snapshot_chunk_X')
              const chunkPromises = [];
              for (let i = 0; i < (snapshot.totalChunks || 1); i++) {
                chunkPromises.push(VrSalesSync.getById(`snapshot_chunk_${i}`));
              }
              ...
              chunks.forEach(c => {
                if (c && c.items) {
                  try {
                    ...
                    items.forEach(item => salesMap[item.codigo] = item);
                  } catch (e) { console.error('Chunk parse error', e); }
                }
              });

            } else if (snapshot.sales) {
              salesMap = JSON.parse(snapshot.sales);
            }

            if (salesMap) {
              const recipeMapKeys = Object.keys(recipeMap);
              let cacheHits = 0;

              recipeMapKeys.forEach(codeKey => {
                const code = parseInt(codeKey);
                const sale = salesMap[codeKey] || salesMap[code]; // Tentar string e number keys
                const recipeIds = recipeMap[code];

                if (sale && recipeIds && recipeIds.length > 0) {
                  // O snapshot contém dados dos últimos 60 dias (configuração do script de sync)
                  // ✅ LÓGICA: "Smart Average" (Média Inteligente)
                  // 1. Dias com venda (evita diluição por dias fechados)
                  // 2. Dia da semana atual (ponderação por recorrência semanal/quinzenal)

                  const daysInPeriod = 60;

                  // ✅ LÓGICA DE SEPARAÇÃO POR LOJA (ROBUSTA - V2)
                  let targetDailySales = sale.daily || {};

                  if (storeId) {
                    const storeIdStr = String(storeId); // Garantir string para comparação

                    // Debugging no Console do Navegador do Usuário
                    // console.log(`🔍 [SuggestionDebug] Product: ${code}, StoreID: ${storeIdStr}, HasStoreData: ${!!(sale.stores && sale.stores[storeIdStr])}`);

                    // Se tem ID de loja, tentar buscar dados específicos desta loja
                    if (sale.stores && sale.stores[storeIdStr] && sale.stores[storeIdStr].daily) {
                      // console.log(`🏪 [Suggestion] Usando dados específicos da Loja ${storeId} para produto ${code}`);
                      targetDailySales = sale.stores[storeIdStr].daily;
                    } else {
                      // Se a loja não vendeu este produto, assumir zero vendas.
                      // IMPORTANTE: Isso evita que lojas sem venda peguem a média global.
                      // console.log(`🏪 [Suggestion] Loja ${storeId} não tem vendas deste produto. Assumindo 0.`);
                      targetDailySales = {};
                    }
                  }

                  const smartStats = this.calculateSmartAverage(targetDailySales, dayOfWeek, daysInPeriod);

                  recipeIds.forEach(recipeId => {
                    consumptionPatterns[recipeId] = {
                      recipe_id: recipeId,
                      statistics: {
                        ...
                        source: `vr_smart_${smartStats.method}` // ex: vr_smart_weekday, vr_smart_weekday_period
                      },
              ...
              if (cacheHits > 0) {
                console.log(`✅ [Suggestion] Cache Firebase: ${cacheHits} itens encontrados (Smart Avg).`);
                sourceInfo = 'vr_real_sales_cache';
                historicalOrdersCount = cacheHits;
              }
            }
          } else {
            console.warn('⚠️ [Suggestion] Snapshot vr_sales_sync vazio ou não encontrado.');
          }

        } catch (err) {
          console.error('❌ [Suggestion] Falha ao processar cache:', err);
        }
      }
      ...
      } else if (sourceInfo === 'vr_real_sales_cache') {
        msg = `${suggestionsApplied} sugestões baseadas em VENDAS REAIS (Cache 30d)`;
      } else {
        msg = dayOfWeek
