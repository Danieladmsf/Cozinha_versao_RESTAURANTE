...
export default function ProductSettingsDialog({ isOpen, onClose, onSave }) {
    ...
    const loadData = async () => {
        try {
            ...
            if (configs && configs.length > 0) {
                ...
                if (config.product_visible_types) {
                    setVisibleTypes(config.product_visible_types);
                } else {
                    const initial = {};
                    types.forEach(t => initial[t.value] = t.value === 'produtos'); // Default to only products
                    setVisibleTypes(initial);
                }
            } else {
                const initial = {};
                types.forEach(t => initial[t.value] = t.value === 'produtos');
                setVisibleTypes(initial);
            }
        } catch (error) {
    ...
    const handleSave = async () => {
        try {
            ...
            const dataToSave = {
                product_visible_types: visibleTypes
            };
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Escolha quais tipos de categorias (abas principais) devem aparecer na tela de Produtos.
                    </DialogDescription>
                </DialogHeader>
                            ...
                            {availableTypes.map(type => (
                                <div key={type.id} className="flex items-center justify-between p-2 border rounded hover:bg-gray-50">
                                    <Label htmlFor={`type-${type.id}`} className="flex-1 cursor-pointer font-medium capitalize">
                                        {type.label || TYPE_LABELS[type.value] || type.value}
                                    </Label>
                                    <Switch
                                        id={`type-${type.id}`}
                                        checked={visibleTypes[type.value] !== false}
                                        onCheckedChange={() => handleToggle(type.value)}
                                        className="data-[state=checked]:bg-blue-600 data-[state=unchecked]:bg-gray-200 border-2 border-transparent transition-colors cursor-pointer"
                                    />
                                </div>
                            ))}
                        </div>
