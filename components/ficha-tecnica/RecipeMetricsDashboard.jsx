...
export default function RecipeMetricsDashboard({ metricsData, className, costFieldName, weightFieldName, variant = 'card' }) {
  ...
  const metrics = {
    ...
    operational_cost: metricsSource?.operational_cost || 0, // COO - Custo Operacional da Operação
  };
  ...
  const metricsConfig = [
    ...
    {
      label: 'Custo Operacional (COO)',
      value: formatDisplayValue(metrics.operational_cost, 'currency'),
      unit: 'R$',
      gradient: 'from-amber-50 to-amber-100',
      borderColor: 'border-amber-200',
      textColor: 'text-amber-600',
      unitColor: 'text-amber-400',
      tooltip: 'Custo Operacional da Operação. Inclui custos de equipamentos, energia e mão de obra utilizados na produção. Diferente do CMV (insumos).',
      listBgHover: 'hover:bg-amber-50 bg-amber-50/10',
      listTextLabel: 'text-amber-600 font-medium',
      listTextValue: 'text-amber-700 font-bold',
      listUnitColor: 'text-amber-400'
    },
    {
      label: 'Tempo de Preparo',
      value: FormatUtils.formatPrepTime((metrics.total_prep_time || 0) / 60),
      unit: '',
      gradient: 'from-cyan-50 to-cyan-100',
      borderColor: 'border-cyan-200',
      textColor: 'text-cyan-600',
      unitColor: 'text-cyan-400',
      tooltip: 'Estimativa de tempo total de mão de obra direta (limpeza/preparo) baseada nos índices técnicos dos insumos.',
      listBgHover: 'hover:bg-cyan-50 bg-cyan-50/10',
      listTextLabel: 'text-cyan-600 font-medium',
      listTextValue: 'text-cyan-700 font-bold',
      listUnitColor: 'text-cyan-400'
    }
  ];
