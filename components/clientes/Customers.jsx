...
import { formatCapitalize, slugify } from "@/lib/textUtils";
...
export default function Customers() {
  ...
  const loadCustomers = async () => {
    try {
      ...
      // Auto-migração: gerar slug para clientes que não possuem
      const customersWithoutSlug = data.filter(c => !c.slug && c.name);
      if (customersWithoutSlug.length > 0) {
        console.log(`🔗 [Migration] Gerando slugs para ${customersWithoutSlug.length} cliente(s)...`);
        for (const c of customersWithoutSlug) {
          const newSlug = slugify(c.name);
          try {
            await Customer.update(c.id, { slug: newSlug });
            console.log(`  ✅ ${c.name} → ${newSlug}`);
          } catch (err) {
            console.error(`  ❌ Erro ao atualizar slug de ${c.name}:`, err);
          }
        }
        // Recarregar para refletir os slugs atualizados
        const updatedData = await Customer.list() || [];
        setCustomers(updatedData);
        console.log('🔗 [Migration] Slugs gerados com sucesso!');
      }
    } catch (error) {
      toast({
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      slug: slugify(formData.get("name")), // Gera slug a partir do nome
    };
              ...
              filteredCustomers.map((customer) => (
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <a
                      href={generateCustomerPortalLink(customer.slug || customer.id)}
                      target="_blank"
