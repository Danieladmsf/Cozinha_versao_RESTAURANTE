...
export default function Recipes() {
  ...
  // Bulk action mode: 'delete' | 'toggleActive' | null
  ...
  const loadVisibilitySettings = async () => {
    try {
      ...
      if (configs && configs.length > 0 && configs[0].recipe_visible_types) {
        ...
        // Default: If no config, we wait for categoryTypes to be loaded or default to empty
        // We will handle the "Show All" logic in the filtering below
        setVisibleTypes({});
      }
    } catch (e) {
  ...
  const visibleCategoryTypes = categoryTypes.filter(type => {
    ...
    if (Object.keys(visibleTypes).length > 0) {
      // If the key exists, respect it.
      return visibleTypes[type.value] === true;
    }

    // Fallback: If no settings at all, show everything (100% dynamic)
    return true;
  });
  ...
  const enterBulkMode = (mode, recipeId, toggleTarget) => {
    setBulkMode(mode);
    setBulkSelected(new Set(recipeId ? [recipeId] : []));
    if (mode === 'toggleActive') setBulkToggleTarget(toggleTarget);
  };
  ...
  const toggleSelectAll = () => {
    if (bulkSelected.size === filteredRecipes.length) {
      setBulkSelected(new Set());
    } else {
      setBulkSelected(new Set(filteredRecipes.map(r => r.id)));
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (failCount === 0) {
      toast({ title: "Receitas excluídas", description: `${successCount} receita(s) excluída(s) com sucesso.` });
    } else {
  ...
  const handleBulkToggleActive = async () => {
    if (bulkSelected.size === 0) return;
    const count = bulkSelected.size;
    const action = bulkToggleTarget ? 'ativar' : 'inativar';
    if (!window.confirm(`Tem certeza que deseja ${action} ${count} receita(s)?`)) return;
    ...
    const actionPast = bulkToggleTarget ? 'ativada(s)' : 'inativada(s)';
    if (failCount === 0) {
      toast({ title: `Receitas ${actionPast}`, description: `${successCount} receita(s) ${actionPast} com sucesso.` });
    } else {
      toast({ title: "Operação parcial", description: `${successCount} ${actionPast}, ${failCount} falhou(aram).`, variant: "destructive" });
    }
  };
  ...
  const getFilteredRecipes = () => {
    ...
    if (activeCategory !== "all") {
      ...
      filtered = filtered.filter(recipe =>
        (recipe.category_id && finalIds.includes(recipe.category_id)) ||
        finalNames.includes(recipe.category)
      );
    }
  ...
  const getEditSelectedCategoryLabel = () => {
    if (!editForm.category) return "Selecione a categoria";
    const found = editGroupedCategories.flatMap(g => g.items).find(c => c.originalName === editForm.category);
  ...
  const openEditModal = (recipe) => {
    ...
    setEditCategorySelectorOpen(false);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="p-4">
          ...
          {bulkMode && (
            <div className={cn(
              "mb-4 flex items-center gap-3 rounded-lg px-4 py-3 shadow-sm border",
              bulkMode === 'delete'
              ...
              <div className={cn("h-5 w-px", bulkMode === 'delete' ? "bg-red-200" : "bg-amber-200")} />
              ...
              >
                {bulkSelected.size === filteredRecipes.length ? (
                  <CheckSquare className="h-4 w-4 mr-1.5" />
                ) : (
                  <Square className="h-4 w-4 mr-1.5" />
                )}
                {bulkSelected.size === filteredRecipes.length ? 'Desmarcar tudo' : 'Marcar tudo'}
              </Button>
              ...
              <span className={cn(
                "text-sm font-medium",
                bulkMode === 'delete' ? "text-red-600" : "text-amber-700"
              )}>
                {bulkSelected.size} selecionada(s)
              </span>
              ...
              {bulkMode === 'delete' ? (
                <Button
                  ...
                  disabled={bulkSelected.size === 0 || isBulkProcessing}
                ...
                >
                  ...
                  {bulkToggleTarget ? 'Ativar' : 'Inativar'} {bulkSelected.size > 0 ? `(${bulkSelected.size})` : ''}
                </Button>
              )}
            </div>
          )}
                ...
                >
                  <Card
                    className={cn(
                      ...
                      bulkMode && isChecked
                        ? bulkMode === 'delete'
                          ? "ring-2 ring-red-400 bg-red-50 shadow-md"
                          : "ring-2 ring-amber-400 bg-amber-50 shadow-md"
                        : "hover:shadow-md",
                      bulkMode && "cursor-pointer"
                    )}
                    onClick={bulkMode ? () => toggleSelectRecipe(recipe.id) : undefined}
                  >
                    <div className="p-4">
                      <div className="flex justify-between items-start">
                        ...
                        {bulkMode && (
                          ...
                          >
                            {isChecked ? (
                              <CheckSquare className={cn("h-5 w-5", bulkMode === 'delete' ? "text-red-500" : "text-amber-500")} />
                            ) : (
                        ...
                        <div className="flex flex-col flex-1 mr-2">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-medium text-base">
                              {recipe.code && <span className="text-orange-600 text-sm font-mono mr-1">#{String(recipe.code).padStart(6, '0')}</span>}
                              {recipe.name}
                            </h3>
                          </div>
                        ...
                        {!bulkMode && (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              ...
                              <DropdownMenuItem
                                onClick={() => enterBulkMode('toggleActive', recipe.id, !recipe.active)}
                                className="flex items-center cursor-pointer"
                              >
                                {recipe.active ? (
                                  <>
                              ...
                              <DropdownMenuItem
                                onClick={() => enterBulkMode('delete', recipe.id)}
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Popover open={editCategorySelectorOpen} onOpenChange={setEditCategorySelectorOpen}>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    role="combobox"
                    aria-expanded={editCategorySelectorOpen}
                    className="w-full justify-between font-normal"
                  >
                    {getEditSelectedCategoryLabel()}
                        ...
                        <CommandGroup key={group.groupName} heading={group.groupName}>
                          {group.items.map((category) => (
                            <CommandItem
                              ...
                              onSelect={() => {
                                setEditForm(prev => ({ ...prev, category: category.originalName }));
                                setEditCategorySelectorOpen(false);
                              }}
                            >
                              <Check
                                className={cn(
                                  "mr-2 h-4 w-4",
                                  editForm.category === category.originalName ? "opacity-100" : "opacity-0"
                                )}
                              />
                              {category.label}
                            </CommandItem>
