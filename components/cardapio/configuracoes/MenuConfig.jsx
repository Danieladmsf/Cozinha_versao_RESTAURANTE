...
export default function MenuConfig({ categories, onConfigChange }) {
  ...
  const [availableDays, setAvailableDays] = useState([0, 1, 2, 3, 4, 5, 6]); // Todos os dias por padrão
  ...
  const loadConfig = async () => {
    try {
      ...
      if (configs && configs.length > 0) {
        ...
        setAvailableDays(config.available_days || [0, 1, 2, 3, 4, 5, 6]);
  ...
  const handleSave = async () => {
    try {
      ...
      const configData = {
        ...
        available_days: Array.isArray(availableDays) ? availableDays : [0, 1, 2, 3, 4, 5, 6],
