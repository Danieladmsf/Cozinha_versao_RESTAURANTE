...
export default function IngredientEditor() {
  ...
  const loadIngredient = async (id) => {
    try {
      ...
      // Restaurar itemType baseado no item_type salvo ou inferir da categoria
      const savedItemType = ingredient.item_type;
      const categoryLower = (ingredient.category || '').toLowerCase();
      const inferredType = savedItemType === 'embalagem' || categoryLower.includes('embala')
        ? 'embalagem'
        : 'ingrediente';

      if (inferredType !== itemType) {
        console.log(`🔄 Restaurando itemType para: ${inferredType} (salvo: ${savedItemType}, categoria: ${ingredient.category})`);
        setItemType(inferredType);
        // Recarregar categorias para o tipo correto
        await loadCategories(inferredType);
      }
